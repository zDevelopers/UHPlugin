 ...
 * termes.
 */
package eu.carrade.amaury.UHCReloaded.modules.scenarii.alliances.commands;

import eu.carrade.amaury.UHCReloaded.modules.scenarii.alliances.AllianceRequest;
...
public class RequestAnswerCommand extends Command
{
    ...
    protected void run() throws CommandException
    {
        if (args.length < 2)
        {
            UR.log(AlliancesModule.class).info("{0} (badly) used the alliance-request-answer directly.", sender.getName());
        ...
        try
        {
            ...
            final AllianceRequest request = UR.module(AlliancesModule.class).getRequestByID(requestID);

            if (request != null)
            {
                try
                {
                    UR.log(AlliancesModule.class).info("{0}: reply from {1}: {2}", request, player.getName(), answer);
                    request.registerApproval(player.getUniqueId(), answer);
                }
                catch (IllegalArgumentException e)
                {
                    UR.log(AlliancesModule.class).warning("{0}: {1} tried to reply {2} but was not in the approvers list.", request, player.getName(), answer);
