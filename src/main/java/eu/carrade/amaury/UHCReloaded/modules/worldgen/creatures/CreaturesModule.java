 ...
 * termes.
 */
package eu.carrade.amaury.UHCReloaded.modules.worldgen.creatures;
...
public class CreaturesModule extends UHModule
{
    /**
    ...
    static
    {
        try
        {
            ...
            CREATURE_TYPE_ENUM_CLASS_FIELD = Arrays.stream(CREATURE_TYPE_ENUM.getDeclaredFields())
                    ...
                    .orElseThrow(() -> new IncompatibleMinecraftVersionException(new Exception("Cannot find the field containing the super class in EnumCreatureType")));

            CREATURE_TYPE_ENUM_CLASS_FIELD.setAccessible(true);

            try
            {
                BIOME_META_CONSTRUCTOR = BIOME_BASE_BIOME_META_CLASS.getConstructor(Class.class, int.class, int.class, int.class);
            }
            catch (final NoSuchMethodException | SecurityException e)
            {
                BIOME_META_CONSTRUCTOR = BIOME_BASE_BIOME_META_CLASS.getConstructor(ENTITY_TYPES_CLASS, int.class, int.class, int.class);
            }
            ...
            if (version == null && hadMaps) version = EntityTypesClassVersion.MAPS;
            if (version == null) throw new IncompatibleMinecraftVersionException(new Exception("EntityTypes class version unsupported"));
            ...
            switch (version)
            {
                ...
                case MAPS:
                    ...
                    for (final Field field : ENTITY_TYPES_CLASS.getDeclaredFields())
                    {
                        ...
                        if (entry.getKey().getClass().equals(String.class) && entry.getValue().getClass().equals(Class.class))
                        {
                ...
                case MINECRAFT_KEYS:
                    ...
                    if (registry == null)
                    {
                        throw new IncompatibleMinecraftVersionException(new NoSuchFieldException("Cannot retrieve the EntityTypes registry."));
                    }
                    ...
                    for (final EntityType entityType : EntityType.values())
                    {
                        ...
                        NMS_ENTITY_CLASSES.put(entityType, (Class<?>) REGISTRY_METHOD_GET.invoke(registry, Reflection.instantiate(MINECRAFT_KEY_CLASS, name)));
                    }
                ...
                case STATIC_ATTRIBUTES:
                    ...
                    final Map<String, EntityType> BY_KEY = Arrays.stream(EntityType.values())
                            .map(entityType -> {
                                try
                                {
                                    return new AbstractMap.SimpleEntry<>((String) Reflection.getFieldValue(entityType, "name"), entityType);
                                }
                    ...
                    {
                        if (!field.getType().equals(ENTITY_TYPES_CLASS)) continue;

                        final Object type = field.get(null);
                        final String key = (String) Reflection.call(type, "d");  // Returns the key (String)
                        final Class<?> clazz = (Class<?>) Reflection.call(type, "c");  // Returns the class (Class<? extends Entity>)
    ...
    protected void onEnable()
    {
        for (final Map rule : Config.SPAWN_RULES)
        {
            try
            {
                ...
                final int minPackSize = ConfigurationValueHandlers.handleIntValue(rule.getOrDefault("minimal_pack_size", -1));
                final int maxPackSize = ConfigurationValueHandlers.handleIntValue(rule.getOrDefault("maximal_pack_size", -1));
            ...
            {
                log().warning("Invalid spawn rule, skipping. {0} (erroneous value: {1}).", e.getMessage(), e.getValue());
            }
        }
    }
     ...
     */
    public void patchAnimalsSpawn(final EntityType entity, final int weight, final int spawnPackMin, final int spawnPackMax)
    {
     ...
     */
    public void patchAnimalsSpawn(final EntityType entity, final int weight, final int spawnPackMin, final int spawnPackMax, final boolean onlyIfPresent, final Biome... biomes)
    {
        if (!hooked) return;  // Incompatible Minecraft version

        final Class<?> nmsEntityClass = NMS_ENTITY_CLASSES.get(entity);
        final Object nmsEntityType = NMS_ENTITY_TYPES.get(entity);

        if (nmsEntityClass == null) return;  // Unsupported entity

        final Enum creatureType = getCreatureType(nmsEntityClass);
        if (creatureType == null) return;  // Non-naturally-spawnable entity
        ...
        for (final Biome biome : patchedBiomes)
        {
            ...
            try
            {
                ...
                for (final Object meta : metas)
                {
                    ...
                    if (!Objects.equals(entityClassOrType, nmsEntityClass) && !Objects.equals(entityClassOrType, nmsEntityType)) continue;  // Not the meta we're looking for

                    if (weight >= 0) Reflection.setFieldValue(WEIGHTED_RANDOM_CHOICE_CLASS, meta, "a", weight);
                    if (spawnPackMin >= 0) Reflection.setFieldValue(meta, "c", spawnPackMin);
                    if (spawnPackMax >= 0) Reflection.setFieldValue(meta, "d", spawnPackMax);
                ...
                if (!found && !onlyIfPresent)
                {
                    ...
                    catch (final IllegalArgumentException | InstantiationException | InvocationTargetException | IllegalAccessException e)
                    {
                        try
                        {
                            ...
                            {
                                throw new IncompatibleMinecraftVersionException(new Exception("Unknown entity type for " + entity));
                            }

                            meta = BIOME_META_CONSTRUCTOR.newInstance(nmsEntityType, weight, spawnPackMin, spawnPackMax);
                        }
                        catch (final IllegalArgumentException | InstantiationException | InvocationTargetException | IllegalAccessException e1)
                        {
                            log().warning("Unable to construct a new BiomeBase.BiomeMeta (tested two constructors). Nag UHCReloaded authors about this.", e1);
