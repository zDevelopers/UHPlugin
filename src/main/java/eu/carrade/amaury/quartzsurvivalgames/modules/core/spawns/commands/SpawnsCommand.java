...
public class SpawnsCommand extends Command
{
    ...
    protected void add() throws CommandException
    {
        ...
        else
        {
            try
            {
                QSG.module(SpawnsModule.class).addSpawnPoint(world, Double.parseDouble(args[1]), Double.parseDouble(args[2]));
    ...
    protected void generate() throws CommandException
    {
        ...
        // Default values
        int size = QSG.module(BorderModule.class).getCurrentBorderDiameter() - 25; // Avoid spawn points being too close to the border
        ...
        if (args.length < 6)
        {
            ...
            if (spawnsCount == 0)
            {
                spawnsCount = Bukkit.getServer().getOnlinePlayers().size() - QSG.module(SpectatorsModule.class).getSpectators().size();
