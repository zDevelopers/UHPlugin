...
package eu.carrade.amaury.quartzsurvivalgames.modules.core.spawns.commands;
...
public class SpawnsCommand extends Command {
    ...
    protected void help() {
        ...
        info(I.t(
                "{cc}/uh spawns dump {ci}: displays the registered spawn points in an exportable format. {gray}Use this to plot the spawn points, as example."));
        info(I.t(
                "{cc}/uh spawns add {ci}: adds a spawn point for a team or a player, at the current location of the sender or at the provided coordinates."));
        info(I.t(
                "{cc}/uh spawns remove [<x> <z>] {ci}: removes the spawn points at the specified coordinates, or at the current location if the sender without coordinates."));
        info(I.t(
                "{cc}/uh spawns generate {ci}: automagically generates spawn points. See /uh spawns generate for details."));
        info(I.t("{cc}/uh spawns reset {ci}: removes all registered spawn points."));
    }

    protected void list() {
        final List<Location> spawnPoints = QSG.module(SpawnsModule.class).getSpawnPoints();
    ...
    protected void add() throws CommandException {
        ...
        if (sender instanceof Player) {
            ...
            if (world == null) {
                throwInvalidArgument(I.t("There is no world named {0}.", args[3]));
            }
        } else {
            world = QSG.get().getWorld(Environment.NORMAL);
        }

        // /uh spawns add
        if (args.length == 1) {
            try {
                QSG.module(SpawnsModule.class).addSpawnPoint(playerSender().getLocation());
                success(I.t("{cs}Spawn added in the world {0}: {1};{2}", world.getName(),
        ...
        // /uh spawns add <x> <z> [world]
            ...
            try {
                QSG.module(SpawnsModule.class)
                        .addSpawnPoint(world, Double.parseDouble(args[1]), Double.parseDouble(args[2]));
    ...
    protected void remove() throws CommandException {
        // /uh spawns remove
        if (args.length == 1) {
            QSG.module(SpawnsModule.class).removeSpawnPoint(playerSender().getLocation(), false);
            success(I
                    .t("{cs}The spawn point {1};{2} in the world {0} was removed.", playerSender().getWorld().getName(),
                            playerSender().getLocation().getBlockX(), playerSender().getLocation().getBlockZ()));
        ...
        // /uh spawns remove <x> <z>
            ...
            try {
                ...
                QSG.module(SpawnsModule.class).removeSpawnPoint(
                        new Location(
    ...
    protected void reset() {
        QSG.module(SpawnsModule.class).reset();
    ...
    protected void generate() throws CommandException {
        ...
        if (args.length == 1) {
            ...
            info(I.t(
                    "{cc}/uh spawns generate <circular|grid|random> [size] [distanceMin] [count] [xCenter] [zCenter] [world]"));
            info("");
            info(I.t("{aqua}Shapes"));
            info(I.t(
                    " - {cc}random{ci}: generates random spawn points on the map, with a minimal distance between them."));
            info(I.t(
                    " - {cc}grid{ci}: generates the spawn points on concentric squares, with a constant distance between two generated points."));
            info(I.t(
                    " - {cc}circular{ci}: generates the spawn points on concentric circles, with a minimal distance between two generated points. In each circle, the angle (and the distance) between two spawn points is constant."));
            info("");
            info(I.t("{aqua} Arguments "));
            info(I.t(
                    " - {cc}size{ci}: the size of the region where the spawn points will be generated. Squared or circular, following the shape of the map. Default: map' size."));
            info(I.t(" - {cc}distanceMin{ci}: the minimal distance between two spawn points. Default: 250 blocks."));
            info(I.t(
                    " - {cc}count{ci}: the number of spawn points to generate. Default: the number of players or teams."));
            info(I.t(
                    " - {cc}xCenter{ci}, {cc}zCenter{ci}: the center of the region where the points are generated. Default: world' spawn point."));
        ...
        // Default values
        int size = QSG.module(BorderModule.class).getCurrentBorderDiameter() -
                25; // Avoid spawn points being too close to the border
        int distanceMinBetweenTwoPoints = 250;
        World world = QSG.get().getWorld(Environment.NORMAL);
        ...
        if (args.length < 6) {
            // Solo mode?
            if (spawnsCount == 0) {
                spawnsCount = Bukkit.getServer().getOnlinePlayers().size() -
        ...
        try {
            QSG.module(SpawnsModule.class)
                    .generateSpawnPoints(generationMethod, world, spawnsCount, size, distanceMinBetweenTwoPoints,
                            xCenter, zCenter);
        }
            ...
            error(I.t(
                    "{ce}You asked for the impossible: there are too many spawn points on a too small surface. Decrease the spawn count or the minimal distance between two points."));
        }
    ...
    protected List<String> complete() throws CommandException {
        if (args.length == 1) {
            ...
            return getMatchingSubset(Bukkit.getWorlds().stream().map(World::getName).collect(Collectors.toList()),
                    args[3]);
