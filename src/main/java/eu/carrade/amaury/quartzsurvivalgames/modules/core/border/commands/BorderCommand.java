    ...
    protected void run() throws CommandException {
        ...
        if (args.length == 0) {
            if (border.getMapShape() == MapShape.CIRCULAR) {
                sender.sendMessage(I.tn("{ci}The current diameter of the map is {0} block.",
                        "{ci}The current diameter of the map is {0} blocks.", border.getCurrentBorderDiameter()));
            } else {
            ...
            if (args.length == 1) {
                try {
                    ...
                    if (border.getPlayersOutside(newDiameter).size() != 0) {
                        sender.sendMessage(
                                I.t("{ce}Some players are outside the future border, so this operation was cancelled."));
                        sender.sendMessage(
                        ...
                        if (border.getMapShape() == MapShape.CIRCULAR) {
                            Bukkit.getServer().broadcastMessage(
                                    I.tn("{lightpurple}The diameter of the map is now {0} block.",
                                            "{lightpurple}The diameter of the map is now {0} blocks.", newDiameter));
                        } else {
                            Bukkit.getServer().broadcastMessage(
            ...
            // /uh border <radius> force
                ...
                try {
                    ...
                    if (border.getMapShape() == MapShape.CIRCULAR) {
                        ...
                        Bukkit.getServer()
                                .broadcastMessage(I.t("{lightpurple}The size of the map is now {0}×{0}.", newDiameter));
                    }
                }
                catch (NumberFormatException e) {
                    error(I.t("{ce}“{0}” is not a number...", args[0]));
            ...
            // /uh border <radius> <duration>
                ...
                if (!border.getBorderProxy().supportsProgressiveResize()) {
                    error(I.t("The border motor ({0}) does not supports progressive resizes.",
                            border.getBorderProxy().getClass().getSimpleName()));
                }
                ...
                try {
                    ...
                    error(I.t("{ce}“{0}” is not a valid time delta... Accepted formats are mm, mm:ss or hh:mm:ss.",
                            args[1]));
                    return;
                }
                ...
                if (border.getMapShape() == MapShape.CIRCULAR) {
                    Bukkit.getServer().broadcastMessage(
                            I.tn("{lightpurple}The diameter of the map will be set to {0} block over {1}.",
                                    "{lightpurple}The diameter of the map will be set to {0} blocks over {1}.",
                                    newDiameter, newDiameter, delta));
                } else {
                    Bukkit.getServer().broadcastMessage(
