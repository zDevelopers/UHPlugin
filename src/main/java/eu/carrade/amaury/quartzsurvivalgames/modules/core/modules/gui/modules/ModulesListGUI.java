...
public class ModulesListGUI extends ExplorerGui<ModuleWrapper>
{
    ...
    protected void onUpdate()
    {
        ...
        {
            setTitle(ChatColor.BLACK + filterCategory.getDisplayName() + ChatColor.DARK_GRAY + String.format(" (%d / %d)", modules.length, QSG
    ...
    protected ItemStack getPickedUpItem(final ModuleWrapper module)
    {
        ...
        else
        {
            if (module.setEnabled(!module.isEnabled()))
            {
                if (module.isEnabled())
                {
                    QSG.log().broadcastAdministrativePrefixed(I.t("{yellow}{0} {green}enabled {yellow}the module {1}.", getPlayer().getName(), module.getName()));
                }
                else
                {
                    QSG.log().broadcastAdministrativePrefixed(I.t("{yellow}{0} {red}disabled {yellow}the module {1}.", getPlayer().getName(), module.getName()));
                }
            }
