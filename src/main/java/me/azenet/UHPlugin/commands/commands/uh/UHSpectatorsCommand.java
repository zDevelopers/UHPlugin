...
public class UHSpectatorsCommand extends AbstractCommand {
	...
	public UHSpectatorsCommand(UHPlugin p) {
		this.p = p;
		this.i = p.getI18n();
	}
	...
	public void run(CommandSender sender, String[] args) throws CannotExecuteCommandException {
		if(args.length == 0) { // /uh spec
			...
			if(subcommand.equalsIgnoreCase("add")) {
				...
				if(args.length == 1) { // /uh spec remove
					throw new CannotExecuteCommandException(CannotExecuteCommandException.Reason.BAD_USE, this);
				}
	...
	public List<String> tabComplete(CommandSender sender, String[] args) {
		...
		else if(args.length == 2 && args[1].equalsIgnoreCase("remove")) { // /... spec remove <?>
